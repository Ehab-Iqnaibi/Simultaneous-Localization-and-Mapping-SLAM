...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      particles(i).landmarks(l).mu(1) = robot(1) + z(j).range * cos(normalize_angle(z(j).bearing + robot(3)));
      particles(i).landmarks(l).mu(2) = robot(2) + z(j).range * sin(normalize_angle(z(j).bearing + robot(3)));
      ...
      particles(i).weight = sqrt(det(2*pi*Q)) * exp(-0.5*error'*inv(Q)*error);
    end
