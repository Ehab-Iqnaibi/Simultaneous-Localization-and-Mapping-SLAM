...
for i = 1:m
  ...
  Z(i*2-1) = z(i).range;
  Z(i*2)   = z(i).bearing;
	%Z(2*i-1:2*i) = [z(i).range; z(i).bearing]; %ehab
  ...
  q = delta' * delta;
  %expectedZ(2*i-1:2*i) = [sqrt(q); normalize_angle(atan2(delta(2), delta(1)) - mu(3))];
...
%	Hi = [(mu(1) - mu(2*landmarkId+2)) / sqrt(q), (mu(2) - mu(2*landmarkId+3)) / sqrt(q), 0;
%         (mu(2*landmarkId+3) - mu(2)) / q,        (mu(2*landmarkId+2) - mu(1)) / q,      -1];
...
innovation = normalize_all_bearings(Z - expectedZ);
