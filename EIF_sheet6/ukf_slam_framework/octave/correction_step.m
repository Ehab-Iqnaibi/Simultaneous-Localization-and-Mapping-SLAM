...
for i = 1:m
	...
	% TODO: Compute z_points (2x2n+1), which consists of predicted measurements from all sigma points
        % This corresponds to line 7 on slide 32
          deltax = landmarkXs - sigma_points(1,:);
          deltay = landmarkYs - sigma_points(2,:);
          q = deltax .^ 2 + deltay .^ 2;
          z_points = [sqrt(q); normalize_angle(atan2(deltay, deltax) - mu(3))];
  ...
  wm_x = cos(z_points(2,:)) * wm';
  wm_y = sin(z_points(2,:)) * wm';

  zm = [sum(wm .* z_points(1,:)), atan2(wm_y,wm_x)]';
	...
	% TODO: Compute the innovation covariance matrix S (2x2), line 9 on slide 32
        ...
        diff_m = z_points - zm;
        diff_m(2,:) = normalize_angle(diff_m(2,:));

        S =  wc.*  diff_m * diff_m' + Q;
	...
	% sigma_x_z is an nx2 matrix, where n is the current dimensionality of mu
        ...
        sigma_x_z = wc .* diff_sigma * diff_m';
